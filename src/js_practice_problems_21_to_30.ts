...
function multiplyValues(x: number, y: number): number {
    return x * y;
}
...
function lastItem(items: Array<string>): string {
...
function repeatWord(word: string, times: number): void {
...
function addToList(list: string[], newItem: string): string [] {
    list.push(newItem);
    return list;
}
...
function isZero(valueToCheck: number): boolean {
...
function boolToString(flag: boolean): string {
    if (flag === true) {
        return 'true';
    } else if (flag === false) {
        return 'false';
