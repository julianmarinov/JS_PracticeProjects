...
function reverseWords(sentenceToRev) {
    let words = sentenceToRev.split(' ');
    let reversed = [];
...
function endsWithPeriod(textToTest) {
...
function multiplyAll(numValues) {
    let result = 1;
    numValues.forEach((item) => {
        result = result * item;
    });
    return result;
}
...
function dashify(phrase) {
    return phrase.split(' ').join('-');
}
console.log(dashify('This is a test phrase.'));
//# sourceMappingURL=js_practice_problems_51_to_60.js.map